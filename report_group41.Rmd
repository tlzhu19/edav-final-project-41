...
those groups that carried out the 1994 Rwanda genocide.', 'blah', 'blah', '\nAlthough Bhutan saw a decline in annual tourism 
...
source of funding for many rebel groups.', 'blah', 'blah', '\nKing Jigme Khesar Namgyel Wangchuck marries 
...
cases reported from the current outbreak in West Africa', 'blah', 'blah', '\nJohn Kerry becomes the first-ever US secretary of state 
...
parliamentary elections to form a government.', 'D\nozens killed in protests against proposed electoral 
law changes which the opposition said were designed 
to allow President Kabila to remain in power.', 'blah', 'blah', 'blah', "\n- Htin Kyaw sworn in as president, ushering in a 
...
the way for a new cabinet to include opposition figures.", 'blah', 'blah', 'blah', '\n- The United Nations human rights council decides 
...
during the year, aid agencies say.', 'blah', 'blah', 'blah', '\n- President Htin Kyaw resigns on health grounds 
...
- Ebola outbreak in the east.', 'blah', 'blah')
...
p_hindu_dropdown <- plot_ly(hindu_df_messy, x = ~Year) %>%
  add_bars(y = ~Bhutan, name = 'Bhutan') %>%
  add_bars(y = ~Burma, name = 'Burma') %>%
  layout(
    title = 'Hindu by Country Over Time', 
...
p_muslim_dropdown <- plot_ly(muslim_df_messy, x = ~Year) %>%
  add_bars(y = ~Burma, name = 'Burma') %>%
  add_bars(y = ~DRC, name = 'DRC') %>%
  add_bars(y = ~Iraq, name = 'Iraq') %>%
  add_bars(y = ~Somalia, name = 'Somalia') %>%
  layout(
    title = 'Muslim by Country Over Time', 
    xaxis = list(domain = c(2009, 2018)), 
    yaxis = list(title = 'y'),
    updatemenus = list(
      list(
